...
object LR_BDL_CD extends Settings {
...
//    val trainingData = rawTrainingData.map(line =>
//      (math.floor(math.random*numSlices).toInt, Preprocess.parseAdsLine(line, featureMap.value))
//     ).groupByKey(numCores).mapValues(seq => seq.toArray)
//    .partitionBy(new HashPartitioner(numCores)).persist(storageLevel)
//    val testingData = sc.textFile(testingInputPath, numCores).map(line =>
//      Preprocess.parseAdsLine(line, featureMap.value)).persist(storageLevel)
    ...
    var wg_dist = trainingData.mapValues(data => 
      Pair(new Array[Float](P),Array.tabulate(P)(i=>gamma_init))).persist(storageLevel)
    ...
    while (iter < MAX_ITER && auc < targetAUC && obj > Double.NegativeInfinity) {
      ...
      val w_stat = wg_dist_updated.mapValues(
            tuple => { var i = 0
                       ...
                       while (i < P) {
                         stat(i) = 
                           if (iter > 0 && bayes) (tuple._1(i)*tuple._2(i), tuple._1(i), tuple._2(i))
                           else (tuple._1(i)*gamma_init, tuple._1(i), gamma_init)
                         i += 1}
                       (seqP, stat)}
      ...
      wg_dist =  wg_dist_updated.mapValues(tuple => (tuple._1, tuple._2))
      val innerIter = wg_dist_updated.map(tuple => tuple._2._4).reduce(_+_)*1.0/numSlices
      val innerObj = wg_dist_updated.map(tuple => tuple._2._3).reduce(_+_)*1.0/numSlices
      ...
      if (w_updated.length < 200) {
        ...
        wg_dist_updated.mapValues(tuple => tuple._2).collect.foreach(
